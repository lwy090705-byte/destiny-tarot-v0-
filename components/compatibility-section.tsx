...
import { pickLabel, type FullLabelRow, getFortuneContentLanguage, type FortuneContentLanguage } from "@/lib/fortune-generator"
...
function getCompatElementLabel(el: string, language: Language): string {
  const L = getFortuneContentLanguage(language)
  const row: Record<FiveEl, Record<FortuneContentLanguage, string>> = {
...
const p1Trait: Record<FortuneContentLanguage, Record<FiveEl, string>> = {
  ko: {
...
const p2Trait: Record<FortuneContentLanguage, Record<FiveEl, string>> = {
  ko: {
    wood: '창의적이고 포용력이 넓',
    fire: '사교적이고 낙천적',
    earth: '책임감이 강하고 현실적',
...
function personalityLine0(L: FortuneContentLanguage, n1: string, n2: string, e1: string, e2: string, el1: FiveEl, el2: FiveEl): string {
...
function buildCompatibilityNarrative(
  ...
  const L = getFortuneContentLanguage(language)
  const e1 = getCompatElementLabel(el1, language)
  const e2 = getCompatElementLabel(el2, language)
        ...
        <div className="bg-white rounded-2xl p-6 shadow-sm border-l-4 border-rose-500 space-y-5">
          ...
          <div className="flex items-center justify-center gap-4">
            <div className="text-center">
              ...
              <p className="text-xs text-rose-500">{getCompatElementLabel(result.element1, language)}</p>
            </div>
            <div className="text-3xl text-pink-400 font-light">♥</div>
            <div className="text-center">
              <p className="font-bold text-gray-800">{person2.name}</p>
              <p className="text-xs text-rose-500">{getCompatElementLabel(result.element2, language)}</p>
            </div>
          </div>
